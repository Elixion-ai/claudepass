...
invalidates it entirely (falling back to the ordinary refusal), and a
path that differs from the one actually run even trivially (a `./`
prefix, different quoting) is a different literal word and is never
matched either. A script whose written body itself reads a Secret file
is still refused, exactly as if that content had come from a real file
on disk.
...
| `$CPASS_HOME/vault.cpv.lock` | The sidecar `flock` every Vault writer holds for its whole Open-mutate-Save cycle (`vault.Update`), and that `vault.Create` also takes around its own exists-check-then-create so two concurrent `cpass init` runs can't both create the Vault and race its data key (CLA-98); never removed, never itself holds any Vault data. macOS/Linux only. | `0600` |
