...
- **Command Policy.** A static rule set evaluated against the command's argv (or, for the PreToolUse hook, its raw shell-string text) before spawning: refuse known reveal-only programs, refuse any argv referencing the temp-dir path with a reader, refuse a Secret-bearing file read, refuse a raw Secret-shaped literal, refuse shell strings whose only content is variable expansion — and fail closed, not open, on a shell-invocation shape it cannot statically parse. Rules are the same in the CLI, the PreToolUse hook, and the MCP server, with the small number of disclosed exceptions `internal/policy`'s own package doc comment and `docs/THREATS.md` state precisely (the hook's own `cpass add`-inline-value check, its raw-literal scan's inclusion of argv[0], and the hook-only `printenv` allowlist for well-known non-secret variable names). See ADR-0013 for what this rule set is — and is not — a decision procedure for. A human-only `--unsafe-allow` flag (refused without a TTY) overrides.
